...
fn load_path_dependency(
    ...
    let swim_toml_path = Path::new(&dep.path).join("swim.toml");
    let manifest_content = fs::read_to_string(&swim_toml_path).expect("Failed to read swim.toml");
    let manifest: Manifest = toml::from_str(&manifest_content).expect("Failed to parse swim.toml");
